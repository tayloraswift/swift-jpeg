...
extension JPEG {
    ...
    public struct JFIF {
        ...
        /// -   Parameter density:
        ///     The sampling density of the image.
        public init(version: Version, density: (x: Int, y: Int, unit: Unit?)) {
    ...
    public struct EXIF {
        ...
        public struct Box {
            ...
            public let contents: (UInt8, UInt8, UInt8, UInt8)
        ...
        /// in one step.
        public private(set) var tags: [UInt16: Int]
...
extension JPEG.JFIF.Version {
    static func parse(code: (UInt8, UInt8)) -> Self? {
...
extension JPEG.JFIF.Unit {
    static func parse(code: UInt8) -> Self?? {
        switch code {
        case 0:
            return .some(nil)
        case 1:
...
extension JPEG.JFIF {
    ...
    public static func parse(_ data: [UInt8]) throws -> Self {
        ...
        guard data[0 ..< 5] == Self.signature[...] else {
            throw JPEG.ParsingError.invalidJFIFSignature(.init(data[0 ..< 5]))
        }

        guard let version: Version   = .parse(code: (data[5], data[6])) else {
            throw JPEG.ParsingError.invalidJFIFVersionCode((data[5], data[6]))
        }
        guard let unit: Unit?        = Unit.parse(code: data[7]) else {
...
extension JPEG.EXIF.FieldType {
    static func parse(code: UInt16) -> Self {
        switch code {
        case 1:
            return .uint8
        case 2:
...
extension JPEG.EXIF {
    static let signature: [UInt8] = [0x45, 0x78, 0x69, 0x66, 0x00, 0x00]
    ...
    public static func parse(_ data: [UInt8]) throws -> Self {
        guard data.count >= 14 else {
            throw JPEG.ParsingError.mismatched(
                marker: .application(1),
                count: data.count, minimum: 14
            )
        }

        // look for 'Exif\0\0' signature
        guard data[0 ..< 6] == Self.signature[...] else {
    ...
    private mutating func index(ifd: Int) {
        guard ifd + 2 <= self.storage.count else {
            return
        }
        ...
        for i: Int in 0 ..< count {
            ...
            guard offset + 12 <= self.storage.count else {
                continue
            }
    ...
    public subscript(tag tag: UInt16) -> (type: FieldType, count: Int, box: Box)? {
        guard let offset: Int = self.tags[tag] else {
            return nil
        }
    ...
    public subscript(offset: Int, as _: UInt8.Type) -> UInt8 {
        self.storage[offset]
    }
    ...
    public subscript(offset: Int, as _: UInt16.Type) -> UInt16 {
        switch self.endianness {
        case .littleEndian:
    ...
    ///     of this metadata record.
    public subscript(offset: Int, as _: UInt32.Type) -> UInt32 {
        switch self.endianness {
        case .littleEndian:
