...
extension Test {
    static var cases: [(name: String, function: Function)] {
        [
            (
                "color-sequential-regression",         .string(
                    Self.test(_:),
                    [
            ...
            (
                "grayscale-sequential-regression",     .string(
                    Self.test(_:),
                    [
            ...
            (
                "color-progressive-regression",        .string(
                    Self.test(_:),
                    [
            ...
            (
                "grayscale-progressive-regression",    .string(
                    Self.test(_:),
                    [
    ...
    static func test(_ path: String) -> Result<Void, Failure> {
        do {
            ...
            guard   let ycc: [JPEG.YCbCr] = try (
                System.File.Source.open(path: "\(path).ycc") {
                    guard let data: [UInt8] = $0.read(count: 3 * output.ycc.count) else {
            ...
            let rgb: [JPEG.RGB]   = try (
                System.File.Source.open(path: "\(path).rgb") {
                    guard let data: [UInt8] = $0.read(count: 3 * output.rgb.count) else {
                ...
                guard let _: Void = try (
                    System.File.Destination.open(path: "\(path).ycc") {
                        guard let _: Void = $0.write(
                            output.ycc.flatMap{ [$0.y, $0.cb, $0.cr] }
                        ) else {
                ...
                guard let _: Void = try (
                    System.File.Destination.open(path: "\(path).rgb") {
                        guard let _: Void = $0.write(
                            output.rgb.flatMap{ [$0.r, $0.g, $0.b] }
                        ) else {
                ...
                throw Failure.init(
                    message: """
                    no golden output for '\(path)' (new golden output written to '\(
                        path
                    ).ycc', '\(
                        path
                    ).rgb')
            ...
            guard output.ycc == ycc, output.rgb == rgb else {
