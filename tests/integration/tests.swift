...
extension Test {
    static var cases: [(name: String, function: Function)] {
        [
            (
                "color-sequential-robustness",         .string(
                    Self.decode(_:),
                    [
            ...
            (
                "color-progressive-robustness",        .string(
                    Self.decode(_:),
                    [
            ...
            (
                "restart-interval-robustness",    .string(
                    Self.decode(_:),
                    [
            ...
            (
                "color-sequential-encoding-robustness", .string_int2(
                    Self.encodeColorSequential(_:_:),
                    [
            ...
            (
                "grayscale-sequential-encoding-robustness", .string_int2(
                    Self.encodeGrayscaleSequential(_:_:),
                    [
                        ("tests/integration/encode/karlie-kloss-1", (640, 320))
                    ]
                )
            ...
            (
                "color-progressive-encoding-robustness", .string_int2(
                    Self.encodeColorProgressive(_:_:),
                    [
                        ("tests/integration/encode/karlie-kloss-1", (640, 320))
                    ]
                )
            ...
            (
                "grayscale-progressive-encoding-robustness", .string_int2(
                    Self.encodeGrayscaleProgressive(_:_:),
                    [
                        ("tests/integration/encode/karlie-kloss-1", (640, 320))
                    ]
                )
    ...
    private static func print(image rgb: [JPEG.RGB], size: (x: Int, y: Int)) {
    ...
    static func decode(_ path: String) -> Result<Void, Failure> {
        do {
            guard let image: JPEG.Data.Rectangular<JPEG.Common> = try .decompress(
                path: path
            ) else {
            ...
            Swift.print(
                ...
                \(Highlight.bold)\(path)\(Highlight.reset) (\(image.layout.format))
                {
                    ...
                    [
                        ...
                        """
                        [band: \(String.pad("\($0.band.lowerBound)", left: 2)) ..< \(
                            String.pad("\($0.band.upperBound)", left: 2)
                        ), bits: \(
                            String.pad("\($0.bits.lowerBound)", left: 2)
                        ) \(
                            String.pad(
                                $0.bits.upperBound == .max ? "..." : """
                                ..< \("\(String.pad("\($0.bits.upperBound)", left: 2))")
                                """,
                                right: 6
                            )
                        )]: \(
            ...
            // write to rgb file
            guard let _: Void = try (
                System.File.Destination.open(path: "\(path).rgb") {
                    guard let _: Void = $0.write(rgb.flatMap{ [$0.r, $0.g, $0.b] }) else {
    ...
    static func encodeColorSequential(_ path: String, _ size: (x: Int, y: Int))
    -> Result<Void, Failure> {
    ...
    static func encodeGrayscaleSequential(_ path: String, _ size: (x: Int, y: Int))
    -> Result<Void, Failure> {
    ...
    static func encodeColorProgressive(_ path: String, _ size: (x: Int, y: Int))
    -> Result<Void, Failure> {
        let format: JPEG.Common              = .ycc8
        let Y: JPEG.Component.Key            = format.components[0],
        Cb: JPEG.Component.Key           = format.components[1],
    ...
    static func encodeGrayscaleProgressive(_ path: String, _ size: (x: Int, y: Int))
    -> Result<Void, Failure> {
        let format: JPEG.Common              = .y8
        let Y: JPEG.Component.Key            = format.components[0]
        let layout: JPEG.Layout<JPEG.Common> = .init(
    ...
    private static func encode(
        ...
        suffix: String,
        size: (x: Int, y: Int),
        layout: JPEG.Layout<JPEG.Common>
    )
    -> Result<Void, Failure> {
        do {
            guard let rgb: [JPEG.RGB]    = try (
                System.File.Source.open(path: "\(path).rgb") {
                    guard let data: [UInt8]  = $0.read(count: 3 * size.x * size.y) else {
            ...
            for (ci, p): (JPEG.Component.Key, KeyPath<JPEG.YCbCr, UInt8>) in
                ...
                guard planar.index(forKey: ci) != nil else {
                    continue
                }
            ...
            guard let _: Void = try spectral.compress(path: "\(path)\(suffix).jpg") else {
