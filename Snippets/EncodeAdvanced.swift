...
for (c, (component, qi)): (
        Int,
        (component: JPEG.Component, qi: JPEG.Table.Quantization.Key)
    ) in layout.planes.enumerated() {
    print("""
        plane \(c)
        {
...
guard let _: Void = try spectral.compress(path: "\(path).jpg") else {
