...
guard let spectral: JPEG.Data.Spectral<JPEG.Common> = try .decompress(path: path) else {
    ...
    print(
        ...
        \((0 ..< 8).map {
                ...
                │ \((0 ..< 8).map {
                        (k: Int) in
                        String.pad(
                            "\(table[z: JPEG.Table.Quantization.z(k: k, h: h)]) ",
                            left: 4
                        )
                    }.joined()) │
                """
...
for (p, plane): (Int, JPEG.Data.Planar<JPEG.Common>.Plane) in planar.enumerated() {
    ...
    guard let _: Void = (
        System.File.Destination.open(path: planepath) {
            guard let _: Void = $0.write(samples) else {
