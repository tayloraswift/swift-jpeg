...
guard let original: JPEG.Data.Spectral<JPEG.Common> = try .decompress(path: path) else {
    ...
    original.read(ci: ci) {
        ...
        recompressed.with(ci: ci) {
            for b: ((x: Int, y: Int), (x: Int, y: Int)) in zip(plane.indices, $0.indices) {
                for z: Int in 0 ..< 64 {
                    let coefficient: Int16 = .init(
                        quanta[z: z]
                    ) * plane[x: b.0.x, y: b.0.y, z: z]
                    let rescaled: Double   = .init(coefficient) / .init($1[z: z])
                    $0[x: b.1.x, y: b.1.y, z: z] = .init(
                        rescaled + (0.3 * (rescaled < 0 ? -1 : 1))
                    )
                }
            }
        }
