...
enum Block {
    ...
    static func transpose(_ input: [Coefficient]) -> [Coefficient] {
        (0 ..< 8).flatMap {
            (y: Int) in
            (0 ..< 8).map {
                (x: Int) in
    ...
    static func reflectVertical(_ input: [Coefficient]) -> [Coefficient] {
        (0 ..< 8).flatMap {
    ...
    static func reflectHorizontal(_ input: [Coefficient]) -> [Coefficient] {
        (0 ..< 8).flatMap {
            (y: Int) -> [Coefficient] in
            (0 ..< 8).map {
                (x: Int) -> Coefficient in
    ...
    static func transform(_ body: ([Coefficient]) -> [Coefficient]) -> [Coefficient] {
...
func rotate(_ rotation: Rotation, input: String, output: String) throws {
    guard var original: JPEG.Data.Spectral<JPEG.Common> = try .decompress(path: input) else {
    ...
    guard let _: Void = try rotated.compress(path: output) else {
...
for argument: String in CommandLine.arguments.dropFirst() {
    if argument.prefix(2) == "--" {
        guard let p: Parameter = Parameter.init(rawValue: .init(argument.dropFirst(2))) else {
        ...
        case .rotation?:
            guard let r: Rotation = Rotation.init(rawValue: argument) else {
                fatalError(
                    """
                    '\(
                        argument
                    )' is not a valid rotation specifier (must be 'ii', 'iii', or 'iv')
                    """
                )
            }
