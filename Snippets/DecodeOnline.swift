...
extension Stream: JPEG.Bytestream.Source {
    ...
    mutating func read(count: Int) -> [UInt8]? {
        guard self.position + count <= data.endIndex else {
            return nil
        }
        guard self.position + count < self.available else {
            self.available += 4096
            return nil
        }
...
guard let data: [UInt8]  = (
    System.File.Source.open(path: path) {
        ...
        guard let count: Int = source.count else {
            return nil
        }
...
func decodeOnline(
    stream: inout Stream,
    _ capture: (JPEG.Data.Spectral<JPEG.Common>) throws -> ()
) throws {
        ...
        case .scan:
            ...
            for index: Int in 0... {
                ...
                guard case .restart(let phase) = marker.type else {
                    ...
                    print(
                        """
                        band: \(scan.band), bits: \(scan.bits), components: \(
                            scan.components.map(\.ci)
                        )
                        """
                    )
...
try decodeOnline(stream: &stream) {
    ...
    zip(last + .init(repeating: .init(0, 0, 0), count: rgb.count - last.count), rgb).map {
        ...
        return .init(
            .init(clamping: 50 * d.r),
            .init(clamping: 50 * d.g),
            .init(clamping: 50 * d.b)
        )
    }
    ...
    guard let _: Void = (
        System.File.Destination.open(path: "\(path)-\(counter).rgb") {
    ...
    guard let _: Void = (
        System.File.Destination.open(path: "\(path)-difference-\(counter).rgb") {
            guard let _: Void = $0.write(difference.flatMap{ [$0.r, $0.g, $0.b] }) else {
